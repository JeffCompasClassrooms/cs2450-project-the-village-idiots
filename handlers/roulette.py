...
blueprint = flask.Blueprint("roulette", __name__)

@blueprint.route('/roulette')
def index():
    """Serves the main roulette game page for the user."""
    db = helpers.load_db()

    # verify user credentials via cookies
    ...
    return flask.render_template('roulette.html',
            title=copy.title, subtitle=copy.subtitle, user=user,username=username,
            games=True)
